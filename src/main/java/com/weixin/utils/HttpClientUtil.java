...
public class HttpClientUtil {
    ...
    public static String sendPostByJson(String url, String body) throws Exception {
		...
		try {
			...
			if (HttpStatus.SC_OK == result.getStatusLine().getStatusCode()) {
				resData = EntityUtils.toString(result.getEntity(),Consts.UTF_8);
			}
		} finally {
			if (result != null) {
				result.close();
    ...
    public static void main(String[] args) {
    	String reqURL = "http://fund.eastmoney.com/f10/F10DataApi.aspx?type=lsjz&code=164818&page=1&per=20";
		String sendGetRequest = HttpClientUtil.sendGetRequest(reqURL, null);
